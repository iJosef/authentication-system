...
exports.registerUser = async (req, res) => {
    ...
    try {
        ...
        if(existingUser) return res.status(400).json({ statusCode: 400, message: "User already exists!" });
...
exports.updateUser = (req, res) => {
    User.updateOne({
        ...
        res.status(201).json({statusCode: 201, message: "Role assigned successfully", data})
    })
}
...
exports.getUser = async (req, res) => {
    try {
        ...
        res.status(200).json({statusCode: 200, message: "user fetched successfully"})
    } catch (err) {
        console.error(err.message);
        res.status(500).send("Server Error")
    }
...
exports.recoverPassword = async (req, res) => {
    ...
    try {
        ...
        const output = `
            ...
            <p><a href="http://localhost:3500/api/auth/change-password?token=${token}">click here</a></p>
        `
        ...
        res.status(200).json({statusCode: 200, message: "Recovery link sent successully"})
    } catch (err) {
        console.error(err.message);
        res.status(500).send("Server Error")
    }
...
exports.changePassword = async (req, res) => {
    ...
    if(!token) return res.status(401).json({statusCode: 401, message: "invalid request"})

    // check if user id is in token
    const decoded = jwt.verify(token, SECRET)
    const user_id = decoded.user.id
    if(!user_id) return res.status(401).json({statusCode: 401, message: "invalid request"})
    ...
    try {
        ...
        res.status(201).json({statusCode: 201, message: "Password updated successfully. Login with new credentials"})
    } catch (err) {
        console.error(err.message);
        res.status(500).send("Server Error")
    }
